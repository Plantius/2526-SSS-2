    ...
    if commits_response.status_code == 200:
        ...
        if commits_data:
            ...
            if patch_response.status_code == 200:
                ...
                if "noreply" not in commiter_email:
...
def fetch_security_page(owner, repo, token):
    ...
    response = requests.get(
        f"https://github.com/{owner}/{repo}/security", headers=headers
    )
...
def extract_email(repo_url, token):
    ...
    for name, method in extraction_methods.items():
        ...
        if name == "REPORT_ADVISORY":
            return {"method": name, "emails": None, "links": None}
        ...
        emails = set(emails)
        links = set(links)
        return {"method": name, "emails": emails, "links": links}
