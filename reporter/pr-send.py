...
def apply_patch_and_create_pr(github_url, patch_file_path, github_token):
    # Extract owner and repo name from the github url
    owner_repo = github_url.split('github.com/')[-1].strip('/')
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        os.chdir(repo.name)
        # Create a new branch
        new_branch_name = 'patch-branch'
        subprocess.run(['git', 'checkout', '-b', new_branch_name], check=True)
        # Apply the patch
        #os.system(f"{cwd}/{patch_file_path} .")
        patch_cwd = gh_url_to_path(url)
        os.chdir(patch_cwd)
        os.system(f"patch --fuzz=3 --ignore-whitespace --verbose "$EXEC_PATH" -i {patch_file_path}")
        os.chdir(tempdir)

	#subprocess.run(['git', 'apply', patch_file_path], check=True)
        # Commit the changes
        subprocess.run(['git', 'commit', '-am', 'Apply patch'], check=True)
    ...
    pr = repo.create_pull(
        ...
        base=repo.default_branch  # You might want to change the base branch if it's different
    )
...
Loading a large file (for example reading /dev/urandom/) can use all the memory within a few seconds and crash the server.""",
    ...
    for i, p in enumerate(projs[skip:]):
        ...
        report = f"""Title: Path traversal Vulnerability in https://github.com/{p['project_name']}
Description:
Due to incorrect usage of `path.join`, https://github.com/{p['project_name']} is vulnerable to Local File Inclusion vulnerability.
