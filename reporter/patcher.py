...
def test_codeql(file_path):
    ...
    with open(file_path, "rb") as fr:
        with open(f"codeql/code/{file_name}", "wb") as tfile:
            tfile.write(fr.read())
    os.system(
        "codeql database create ./codeql/db -s ./codeql/code -l javascript --overwrite >/dev/null 2>/dev/null"
    )
    os.system("rm /tmp/codeqlout 2>/dev/null ; rm /tmp/codeqlout.txt 2>/dev/null")
    os.system(
        "codeql query run -d ./codeql/db  ./codeql/querypack/query-only-sanitize.ql -o /tmp/codeqlout >/dev/null 2>/dev/null ; codeql bqrs info /tmp/codeqlout > /tmp/codeqlout.txt"
    )
...
def validate_patch(github_link, exec_path, app_id, patch_filename, initial_n):
    ...
    result = subprocess.run(
        f"timeout 1200 bash ./run-local.sh {github_link} {exec_path} {app_id} {patch_filename}",
        shell=True,
        text=True,
    )
    ...
    if exit_code == EXIT_CODE_NOT_VULNEABLE:
        ...
        if patched_n > initial_n:
            logging.info(
                f"Static analysis done. Patch is OK. {patched_n} > {initial_n}"
            )
            return exit_code
        else:
            logging.info(
                f"Static analysis done. Patch failed. {patched_n} < {initial_n}"
            )
...
def main():
    ...
    if os.path.exists("./static-analysis-test.ts"):
        os.unlink("./static-analysis-test.ts")
    result = subprocess.run(
        f"timeout 1200 bash ./run-local.sh {github_link} {exec_path} {app_id}",
        shell=True,
        text=True,
    )
    ...
    if exit_code != 0:
        ...
        logging.info(
            "Project was not vulnerable in first run, or didn't run! It shouldn't happen"
        )
    ...
    for llm_try_count in range(llm_try_count + 1, llm_try_count + MAX_PATCH_TRIES + 1):
        ...
        exit_code = validate_patch(
            github_link, exec_path, app_id, patch_filename, initial_n
        )

        db.set_field(proj_id, "llm_try_count", llm_try_count)
        if exit_code != EXIT_CODE_NOT_VULNEABLE:
            logging.info(
                f"The patch failed! Project is still vulnerable(exit code: {exit_code})"
            )
            continue
        ...
        if last_run_method != new_run_method:
            logging.info(
                f"Run method is changed! Last is '{last_run_method}', new method is {new_run_method}"
            )
            continue
