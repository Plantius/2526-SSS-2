...
def get_commit_history(
    owner: str, repo: str, file_path: str, headers: dict
) -> List[Dict]:
...
def get_file_content_at_commit(
    owner: str, repo: str, commit_sha: str, file_path: str, headers: dict
) -> Optional[str]:
...
def find_first_commit_date_with_keyword(
    file_url: str, keyword: str, token: str
) -> Optional[str]:
    ...
    for commit in commits:
        ...
        try:
            ...
            file_content = get_file_content_at_commit(
                owner, repo, commit_sha, file_path, headers
            )
            print(commit_sha)
...
def find_first_commited_date(proj):
    for keyword in ["path.join", "createServer"]:
        commit_date = find_first_commit_date_with_keyword(
            proj["file_github_url"], keyword, GITHUB_TOKEN
        )
        if commit_date:
            return commit_date
    return None
