...
def is_still_vulnerable(proj, fork_url):
    ...
    logging.info(
        f"Verifying name: {proj_name}, filename: {proj_filename}, execpath: {exec_path}"
    )
    result = subprocess.run(
        f"bash ./run-local-verify.sh {github_link} {exec_path} {app_id}",
        shell=True,
        text=True,
    )
...
def check_already_sent_pull_request(repo_name):
    r = requests.get(
        f"https://api.github.com/search/issues?per_page=1&q=is:pr author:{AUTHOR_GH_NAME} repo:{repo_name} {PULL_REQUEST_TITLE}",
...
def check_already_opened_an_issue(repo_name):
    ...
    if r.status_code != 200:
        return -1
    r = r.json()
    if "total_count" not in r or r["total_count"] == 0:
        return -2
    return r
...
def send_pull_req(proj):
    ...
    if already_sent == -1:
        failed_pullreq(
            proj["id"],
            PAUSED_UNKNOWN_ERROR_GHAPI,
            "Failed to get the latest issues from GH",
        )
        return
    elif already_sent != 0:
        failed_pullreq(
            proj["id"],
            PAUSED_VULN_ALREADY_REPORTER,
            f"Already reported to {repo_name}!",
        )
        return
    already_sent = check_already_opened_an_issue(repo_name)
    # print(already_sent)
    if (
        False and already_sent == -1
    ):  # We don't have to pause projects that don't have list of issues.
        failed_pullreq(
            proj["id"],
            PAUSED_UNKNOWN_ERROR_GHAPI,
            f"Failed to get the latest issues from GH",
        )
        return
    elif already_sent != -2:
    ...
    if response.status_code == 202:
        ...
        failed_pullreq(
            proj["id"], PAUSED_UNKNOWN_ERROR_GHAPI, "Failed to fork the repo!"
        )
        return
    ...
    if retval != 0:
        ...
        failed_pullreq(
            proj["id"],
            PAUSED_PROJECT_NOT_VULNERABLE_ANYMORE,
            f"Project isn not vulnerable anymore.",
        )
        return
    ...
    if exit_code != 5:
        ...
        failed_pullreq(
            proj["id"],
            PAUSED_PATCHING_FAILED,
            f"Patching failed. exit code: {exit_code}",
        )
    ...
    try:
        with open(f"./webapp-local{patched_file_path}", "rb") as patched_file:
            ...
            if not upload_file_to_project(
                forked_project["full_name"], patched_file_path, patched_file_buf
            ):
                failed_pullreq(
                    proj["id"],
                    PAUSED_UNKNOWN_ERROR_GHAPI,
                    "Uploading file to forked repo fail",
                )
                return
            else:
        ...
        failed_pullreq(
            proj["id"],
            PAUSED_UNKNOWN_ERROR_EXCEPTION,
            f"Something bad happened during applying patch",
        )
        logging.error(e)
        return
    ...
    r = requests.post(
        f"https://api.github.com/repos/{proj['project_name']}/pulls",
        headers={
            "Authorization": f"token {GITHUB_TOKEN}",
            "Accept": "application/vnd.github+json",
        },
...
if __name__ == "__main__":
    ...
    if (
        input(
            "About to send pull requests from your account. Do you want to proceed? (Y/N)"
        ).lower()
        != "y"
    ):
        exit()
